...
router.get('/:id/:sur/person', async (req, res) => {
	...
	try {
		const survey = await Survey.findById(req.params.sur).populate(populate)
		const entries = await Entry.find({ user: req.params.id, survey : req.params.sur })
		// console.log(entries)
		let result = 0
		...
		res.status(200).json(
			ResultViewUser(survey, populate, result, qRes)
		)
	} catch (err) {
		console.log(err)
		res.status(500).send(err)
...
router.post('/', authAdmin, async (req, res) => {
	const { title, questions, description, options, status, type } = req.body
	// console.log(options)
	try {
		if (!title)
...
router.put('/update/:id', authAdmin, async (req, res) => {
	...
	const { title, questions, description, status,options, type } = req.body
	try {
